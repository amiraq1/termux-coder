...
def test_not_repo_then_init(tmp_path):
    ...
    assert "not a git repository" in asyncio.run(gittool.git_status(gittool.GitEmptyArgs(), ctx))
    out = asyncio.run(gittool.git_init(gittool.GitEmptyArgs(), ctx))
    assert "Initialized" in out or "initialized" in out


def test_checkpoint_commit_restore_flow(tmp_path):
    ctx = make_ctx(tmp_path)
    asyncio.run(gittool.git_init(gittool.GitEmptyArgs(), ctx))

    (tmp_path / "a.py").write_text("x = 1\n")
    assert "checkpoint" in asyncio.run(gittool.git_checkpoint(gittool.GitEmptyArgs(), ctx))
    assert "clean" in asyncio.run(gittool.git_checkpoint(gittool.GitEmptyArgs(), ctx))

    (tmp_path / "a.py").write_text("x = 2\n")
    out = asyncio.run(gittool.git_commit(gittool.GitCommitArgs(message="update x"), ctx))
    assert out.startswith("committed ")

    log = asyncio.run(gittool.git_log(gittool.GitEmptyArgs(), ctx))
    assert "agent: update x" in log  # البادئة إلزامية

    (tmp_path / "a.py").write_text("x = 999\n")
    assert "restored" in asyncio.run(gittool.git_restore(gittool.GitRestoreArgs(paths=["a.py"]), ctx))
...
def test_rejected_restore_keeps_file(tmp_path):
    ...
    asyncio.run(gittool.git_checkpoint(gittool.GitEmptyArgs(), ctx))

    ctx.ui.approve = False
    (tmp_path / "a.py").write_text("v2\n")
    out = asyncio.run(gittool.git_restore(gittool.GitRestoreArgs(paths=["a.py"]), ctx))
...
def test_restore_path_jail(tmp_path):
    ctx = make_ctx(tmp_path)
    asyncio.run(gittool.git_init(gittool.GitEmptyArgs(), ctx))
    out = asyncio.run(gittool.git_restore(gittool.GitRestoreArgs(paths=["../evil"]), ctx))
