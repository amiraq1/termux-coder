...
| `LiveNetworkProbe` | Performs one explicit bounded read-only search probe for Doctor | Runs only with `--network`; returns summary metadata, not page content |
...
The `provider_health` Doctor check constructs the configured provider stack and reads `ProviderHealth` plus non-sensitive resilience configuration without calling `search`. It reports `network_probe.performed=false`; live connectivity is a separate explicit phase. When `--network` is explicitly supplied, `LiveNetworkProbe` performs exactly one bounded read-only search through the configured provider and reports status, duration, provider, result count, and truncation only. It does not persist web content or grant mutation permissions.
...
Web-search tests cover Network Policy modes, provider parsing, response limits, total timeout, approval rejection, and untrusted result output. Capability tests cover explicit registration, duplicate rejection, official-domain filtering, and legacy fallback. Secret-scrubber tests cover known credential patterns, sensitive fields, input immutability, and AuditLog persistence redaction. Policy tests cover GRANULAR automatic reads/search/verification, approval for writes/deletes/general commands, blocked pipelines, and environment configuration.
 Fetch-page tests cover SSRF blocking, private redirects, content-type rejection, bounded extraction, and RESEARCHING orchestration. Doctor tests cover report serialization, English-only output, check isolation, bounded timeouts, duplicate registration, and concurrent registry runs with failure/timeout interleaving. Symbol tests cover AST extraction, ambiguity rejection, signature checks, workspace boundaries, TOCTOU hashes, narrow diffs, approval, and orchestration. Future integration tests must cover:
