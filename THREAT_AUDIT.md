...
**Generated:** 2026-08-18T14:48:51.125629+00:00
**Collected tests:** `256`
...
| T4 | Secrets written to audit logs | **IMPLEMENTED** | `src/termux_coder/security/scrubber.py`, `src/termux_coder/security/audit.py` | `tests/test_scrubber.py` | `PASS` | `7b0d43b 2026-08-18` | Known credential patterns are covered; arbitrary custom secret formats still require review. |
| T5 | Patch corruption and incomplete rollback | **IMPLEMENTED** | `src/termux_coder/tools/edit.py`, `src/termux_coder/tools/patch.py` | `tests/test_patch.py`, `tests/test_patch_plan.py` | `PASS` | `ee3b7a8 2026-08-18` | Patch rollback exists; a separate persistent ConfigManager is not present. |
...
| T9 | YOLO or global auto-approval bypass | **IMPLEMENTED** | `src/termux_coder/security/policy.py` | `tests/test_policy.py` | `PASS` | `20eea12 2026-08-18` | Negative assurance also needs a static scan for bypass flags and aliases. |
...
| T14 | Approval replay or argument tampering | **IMPLEMENTED** | `src/termux_coder/models/contracts.py`, `src/termux_coder/core/orchestrator.py` | `tests/test_orchestrator.py`, `tests/test_orchestrator_integration.py` | `PASS` | `20eea12 2026-08-18` | The implementation uses call fingerprints; there is no separate ApprovalToken type. |
